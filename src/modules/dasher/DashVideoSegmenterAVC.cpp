...
DashVideoSegmenterAVC::DashVideoSegmenterAVC(std::chrono::seconds segDur, std::chrono::microseconds offset) : 
DashVideoSegmenter(segDur, VIDEO_CODEC_AVC, offset)
{
...
VideoFrame* DashVideoSegmenterAVC::parseNal(VideoFrame* nal)
{
    ...
    if ((nalType == AUD_AVC || nal->getFrameTime() > tmpFrame->getFrameTime()) && tmpFrame->getLength() > 0){
    ...
    if ((nalType == IDR || nalType == NON_IDR) &&
        !appendNalToFrame(tmpFrame, nalData, nalDataLength, nal->getWidth(), 
                          nal->getHeight(), nal->getPresentationTime(),
                          nal->getDecodeTime())) { 
